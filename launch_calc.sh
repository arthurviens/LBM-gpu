#!/bin/bash

for nx in 500 750 1000 1250 1500 1750 2000 2250 2500 2750 3000 3250 3500 3750 4000
do
	python python/lbmFlowAroundCylinder-Numba.py -nx ${nx} -ny 300 --profile > logs/numba/run_nx${nx}_ny300_i2000.log
done

for ny in 200 350 500 750 1000 1250 1500 1750 2000 2250 2500 2750 3000 3250 3500 3750 4000
do
	python python/lbmFlowAroundCylinder-Numba.py -nx 420 -ny ${ny} --profile > logs/numba/run_nx420_ny${ny}_i2000.log
done

for iter in 1000 1500 2000 2500 3000 3500 4000 4500 5000
do
	python python/lbmFlowAroundCylinder-Numba.py -nx 420 -ny 300 -i ${iter} --profile > logs/numba/run_nx420_ny300_i${iter}.log
done
